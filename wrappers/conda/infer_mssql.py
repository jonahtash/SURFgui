...
def populate_fragments(in_term_table, frag_table, new_term_table):
    cur.execute('DROP TABLE IF EXISTS '+frag_table)
    cur.execute('DROP TABLE IF EXISTS '+new_term_table)
    #cur.execute('CREATE INDEX idx on Terms(full_normalized)')
    cur.execute('CREATE TABLE '+frag_table+' (randr_fragment varchar(max), original varchar(max), ID varchar(max))')
    cur.execute('CREATE TABLE '+new_term_table+' (randr_fragment varchar(max), randr_fragment_broken varchar(max), term_randr_found varchar(max), ID varchar(max))')
...
def make_type_1(in_term_table, frag_table, new_term_table):
    ...
    for row in cur.fetchall():
        ...
        cur.execute('INSERT INTO '+new_term_table+' SELECT ?, ?, full_normalized, ID FROM '+in_term_table+' WHERE '+_make_like(pieces), (row[0], ' '.join(pieces)))
...
def make_type_2(in_term_table, frag_table, new_term_table):
    ...
    for row in cur.fetchall():
        ...
        for ID in cur.fetchall():
            cur.execute('INSERT INTO '+new_term_table+' VALUES (?, ?, ?, ?)', (row[0], ' '.join(pieces), '', ID[0]))
