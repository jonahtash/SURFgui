...
def _process_line(line):
    ...
    p = subprocess.Popen("ruby ./wrappers/python/pubmedid2pdf.rb "+line,shell = False,
...
def run_id_ruby(file_path,kickback_path,num_threads=8):
...
def _get_liebert(id_txt,out_txt, not_out_txt,num_threads=8):
    pool = Pool(num_threads)
...
def sort_url(id_txt,out_csv,num_threads=8):
    pool = Pool(num_threads)
...
def _get_ox_paywall(id_txt,out_txt,num_threads=8):
    pool = Pool(num_threads)
...
def get_abstracts_mp(txt_ids_in,csv_abstracts_out,num_threads=8):
    pool = PoolMP(num_threads)
...
def get_abstracts(txt_ids_in,csv_abstracts_out,num_threads=8):
    pool = Pool(num_threads)
...
def _download_pdf_errors(pdf_output_dir,kickback_dir,line):
    a = line.split('+')
    if len(a) < 2:
        return
    download_url = "http://www.ncbi.nlm.nih.gov/pmc/articles/"+a[0]+"/pdf/"
    pmed_id = a[1]
    print(download_url)
...
def run_download_program(in_csv, out_pdf, **kwargs):
    in_csv = in_csv.strip()
    out_pdf = out_pdf.strip()
    temp_dir = "./wrappers/conda/temp/"
    pmed_result = kwargs.get("pmed_result")
    pmc_ids = kwargs.get("pmc_ids")
    pmed_ids = kwargs.get("pmed_ids")
    do_make_ids = "T" if not kwargs.get("do_make_ids") else None
    do_pmc_dl = "T" if not kwargs.get("do_pmc_dl") else None
    do_pmed_dl = "T" if not kwargs.get("do_pmed_dl") else None
    if not pmed_result:
        pmed_result = temp_dir+"pubmed_result.csv"
        pmed_result_inaccess = temp_dir + "pubmed_result_inaccessable.csv"
        print("Filtering out unreachable documents..")
        sort_inacessable_csv(in_csv, pmed_result, pmed_result_inaccess)
    if not pmc_ids:
        pmc_ids = temp_dir+"pmc_ids.txt"
    if not pmed_ids:
        pmed_ids = temp_dir+"pmc_ids.txt"
    if do_make_ids and do_make_ids.lower() != 'false':
        print("Seperating ids...")
        get_pmcid_csv(pmed_result,pmc_ids,pmed_ids)
    if do_pmc_dl and do_pmc_dl.lower() != 'false':
        print("Downloading from PMC...")
        get_error(pmc_ids,out_pdf,temp_dir+"failed/")
    if do_pmed_dl and do_pmed_dl.lower() != 'false':
        print("Downloading from PubMed...")
        run_id_ruby(pmed_ids,temp_dir+"pmed_failed_ids.txt")
#run_download_program('D:/Jonah/Documents/NIST/misc/inputs/pubmed_result.csv', './temp/pdf/')
